...
$(".hero__slider").owlCarousel({
    ...
    proQtyCart.prepend('<a href="<%=request.getContextPath()%>/removeToCart?maSP=${p.maSP}&quantity-cart=1"><span class="dec qtybtn">-</span></a>');
    proQtyCart.append('<a href="<%=request.getContextPath()%>/addToCart?maSP=${p.maSP}"><span class="inc qtybtn">+</span></a>');
    proQtyCart.on('click', 'a .qtybtn', function () {
        var $button = $(this);
        var oldValue = $button.parent().find('input').val();
        if ($button.hasClass('inc')) {
            var newVal = parseFloat(oldValue) + 1;
...
//     var flag = true;
//
//     var password  = getValue('user-pass') ;
//     if (password === '' || password.length < 8){
//         flag = false;
//         showError('user-pass', 'Vui lòng kiểm tra lại Password')
//     }
//
//     var email = getValue('user-email');
//     var mailFormat = /^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/
//     if(!mailFormat.test(email)){
//         flag = false;
//         showError('user-email', 'Vui lòng kiểm tra lại Email')
//     }
//     return flag;
// }